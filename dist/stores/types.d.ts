import { useFind } from '../use-find-get/use-find';
import { AnyData, Paginated, Params, QueryInfo } from '../types.js';
...
export type PaginationStateQuery = {
    [pageId: string]: PaginationStatePage;
} & {
    queryParams: Query;
    total: number;
};
export type PaginationStateQid = {
    [qid: string]: PaginationStateQuery;
} & {
...
export type HandleEventsFunction<M extends AnyData> = (item: M, ctx: {
    model: M;
    models: any;
}) => any;
...
export type beforeWriteFn<M extends AnyData> = (item: M) => M | Partial<M>;
export type AssignFn<M extends AnyData> = (dest: M, src: M) => M | Partial<M>;
export {};
